        ...
        struct device_attribute *attr, char *buf)
{
...
#ifdef CTS_SWAP_X_Y
    ret = cts_swap_x_y(cts_dev, rawdata);
    if (ret) {
    ...
    if (data_valid) {
        ...
        for (r = 0; r < rows && count < PAGE_SIZE; r++) {
            for (c = 0; c < cols && count < PAGE_SIZE; c++)
                count += snprintf(buf + count, PAGE_SIZE - count - 1,
                    DATA_FORMAT_STR, rawdata[r * cols + c]);

            buf[count++] = '\n';
        }
    }

    cts_info("FORMAT_DATA_SIZE: %d, count: %d", PAGE_SIZE, count);
        ...
        struct device_attribute *attr, char *buf)
{
...
#ifdef CTS_SWAP_X_Y
    ret = cts_swap_x_y(cts_dev, diffdata);
    if (ret) {
        cts_err("Allocate memory for tmpdata failed");
        goto err_free_diffdata;
    }
    ...
    if (data_valid) {
        ...
        for (r = 0; r < rows; r++) {
            for (c = 0; c < cols; c++)
                count += snprintf(buf + count, PAGE_SIZE - count,
                    DATA_FORMAT_STR, diffdata[r * cols + c]);
        ...
        struct device_attribute *attr, char *buf)
{
...
#ifdef CTS_SWAP_X_Y
    ret = cts_swap_x_y(cts_dev, (u16 *)manualdiff);
    if (ret) {
        cts_err("Allocate memory for tmpdata failed");
        goto err_free_manualdiff;
    }
    ...
    if (data_valid) {
        ...
        for (r = 0; r < rows; r++) {
            for (c = 0; c < cols; c++)
                count += snprintf(buf + count, PAGE_SIZE - count,
                    DATA_FORMAT_STR, manualdiff[r * cols + c]);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (rawdata_min == NULL) {
        cts_err("Allocate memory for rawdata_min failed");
        ret = -ENOMEM;
        goto err_free_rawdata;
    }
    rawdata_max = (s16 *) kmalloc(RAWDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
    if (rawdata_max == NULL) {
        cts_err("Allocate memory for rawdata_max failed");
        ret = -ENOMEM;
...
#ifdef CTS_SWAP_X_Y
        ret = cts_swap_x_y(cts_dev, (u16 *)rawdata);
        if (ret) {
            cts_err("Allocate memory for tmpdata failed");
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret) {
        cts_err("Allocate mem for tmpdata failed\n");
        goto err_free_cap;
    }
#endif
