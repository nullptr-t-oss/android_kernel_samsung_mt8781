...
#define SHORT_TEST_FRAME                    5
#define NOISE_TEST_FRAME                    3
/*Tab A9 code for AX6739A-953 by suyurui at 20230609 end*/
#define OEM_OF_DEF_PROPVAL_RAWDATA_FRAMES   1
#define OEM_OF_DEF_PROPVAL_RAWDATA_MIN      100
#define OEM_OF_DEF_PROPVAL_RAWDATA_MAX      10000
#define OEM_OF_DEF_PROPVAL_NOISE_FRAMES     50
#define OEM_OF_DEF_PROPVAL_NOISE_MAX        1000
...
static int alloc_sleftest_data_mem(struct cts_oem_data *oem_data, int nodes)
{
    ...
    /*Tab A9 code for AX6739A-953 by suyurui at 20230609 start*/
    ALLOC_TEST_DATA_MEM(short, nodes * 2 * SHORT_TEST_FRAME);
    /*Tab A9 code for AX6739A-953 by suyurui at 20230609 start*/
...
static int dump_tsdata_to_csv_file(const char *filepath, int flags,
    const u16 *data, int frames, int rows, int cols, int hw_rows, int hw_cols)
{
    ...
    cts_info("Dump tsdata to csv file: '%s' flags: 0x%x data: %p frames: %d row: %d col: %d hw_row: %d hw_col: %d",
        filepath, flags, data, frames, rows, cols, hw_rows, hw_cols);
    ...
    for (i = 0; i < frames; i++) {
        ...
        data += (hw_rows * hw_cols - rows * cols);
...
{
    static char time_data_buf[TIME_DATA_BUF_LEN] = {0};
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    hw_rows = oem_data->cts_data->cts_dev.hwdata->num_row;
    ...
    if (oem_data->test_rawdata) {
        ret = dump_tsdata_to_csv_file(filepath,
            ...
            oem_data->rawdata_test_frames, rows, cols, hw_rows, hw_cols);
        if (ret < 0) {
    ...
    if (oem_data->test_noise) {
        ret = dump_tsdata_to_csv_file(filepath,
            ...
            (oem_data->noise_test_frames + NOISE_TEST_FRAME), rows, cols, hw_rows, hw_cols);
        if (ret < 0) {
    ...
    if (oem_data->test_short) {
        ret = dump_tsdata_to_csv_file(filepath,
            ...
            SHORT_TEST_FRAME, rows, cols, hw_rows, hw_cols);
        if (ret < 0) {
...
static int cts_rawdata_show(struct seq_file *m, void *v)
{
    ...
    for (i = 0; i < cts_dev->fwdata.rows; i++) {
        for (j = 0; j < cts_dev->fwdata.cols; j++) {
            seq_printf(m, "%5d", data[i * cts_dev->fwdata.cols + j]);
        }
        seq_printf(m, "\n");
    }
...
/*Tab A9 code for AX6739A-1173 by suyurui at 20230615 start*/
int cts_ito_test(void) {
    struct chipone_ts_data *cts_data = g_cts_data;
    struct cts_oem_data *oem_data = NULL;
    int ret = 0;

    if (cts_data->cts_dev.rtdata.suspended == true) {
        cts_err("tp is supended,exit ito");
        return 1;
    }
...
/*Tab A9 code for AX6739A-1173 by suyurui at 20230615 end*/
