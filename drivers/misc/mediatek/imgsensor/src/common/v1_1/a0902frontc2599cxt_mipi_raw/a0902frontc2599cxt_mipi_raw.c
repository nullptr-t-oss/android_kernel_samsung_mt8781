...
#include "a0902frontc2599cxt_mipi_raw.h"
...
/*Tab A9 code for AX6739A-1578 by jianghongyan at 20230711 start */
static kal_uint16 g_tmp0 = 0x50;
static kal_uint16 g_tmp1 = 0x44;
static kal_uint16 g_tmp2 = 0x9f;
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = A0902_FRONT_C2599CXT_SENSOR_ID ,

    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 90000000,            //record different mode's pclk//84
        .linelength = 2440,            //record different mode's linelength
...
/*Tab A9 code for AX6739A-1578 by jianghongyan at 20230711 end */
...
static void set_shutter(kal_uint16 shutter)
{
    ...
    write_cmos_sensor(0xe01a,g_tmp0);
	write_cmos_sensor(0xe01d,g_tmp1);
	write_cmos_sensor(0xe020,g_tmp2);
    ...
    printk("@CSTA_001:set_shutter(),R,0202=0x%x,0203=0x%x\n",read_cmos_sensor(0x0202),read_cmos_sensor(0x0203));
	C2599MIPISENSORDB("@CSTA_001:set_shutter(),W: shutter =%d, framelength =%d,mini_framelegth=%d\n", shutter,imgsensor.frame_length,imgsensor.min_frame_length);
}
/*Tab A9 code for AX6739A-1578 by jianghongyan at 20230711 end */
...
#define AGAIN_NUM	49
	...
	{
	  0x44, 0x44,	0x44,	0x44,	0x44,	0x44,	0x44,	0x44,	0x44,	0x44,	0x4c,	0x4e,	0x4d,	0x4d,	0x4d,	0x4c,	0x4e,	0x4e,	0x4e,
	  0x4e,	0x3e,	0x3e,	0x3c,	0x3c,	0x3e,	0x3e,	0x3d,	0x3d,	0x3d,	0x2d,	0x2c,	0x2d,	0x4d,	0x4d,	0x4d,	0x4d,	0x3c,	0x3d,
	  0x3c,	0x3d,	0x3d,	0x3d,	0x3e,	0x3e,	0x3e,	0x3e,	0x3d,	0x3d,	0x3d,
	};
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 tmp0 = 0, tmp1 = 0, tmp2 = 0,  i = 0, Again_base = 0; // tmp6 = 0,
	kal_uint16 iGain=gain;
    C2599MIPISENSORDB("[lj c2599] C2599 isp again = %d\n",gain);
	...
	// write_cmos_sensor(0x340f,0x13); //group delay 1frame write gain
    printk("@CSTA_001:set_gain(),W,32a9=0x%x,32ac=0x%x,32ad=0x%x\n",tmp0,tmp1,tmp2);
    printk("@CSTA_001:set_gain(),R,32a9=0x%x,32ac=0x%x,32ad=0x%x\n",read_cmos_sensor(0x32a9),read_cmos_sensor(0x32ac),read_cmos_sensor(0x32ad));


	return again_table[Again_base];
}
/*Tab A9 code for AX6739A-1578 by jianghongyan at 20230711 end */
...
/*Tab A9 code for AX6739A-1971 by pengxutao at 20230714 start*/
static void sensor_init(void)
{
     ...
     write_cmos_sensor(0x3280,0x2c);
     write_cmos_sensor(0x3284,0xe3);
     write_cmos_sensor(0x3288,0x40);
     write_cmos_sensor(0x328b,0x61);
     write_cmos_sensor(0x328C,0x48);
     write_cmos_sensor(0x32aa,0x04); //brighter 2023.6.13
     ...
     write_cmos_sensor(0x3298,0x46);
     write_cmos_sensor(0x32af,0x80);
     write_cmos_sensor(0x3212,0x4A);
     write_cmos_sensor(0x3287,0x48);
     ...
     write_cmos_sensor(0x32ac,0x44);
     write_cmos_sensor(0x32ad,0x9f);
     write_cmos_sensor(0x3211,0x15);
     write_cmos_sensor(0x3216,0x1a);
     write_cmos_sensor(0x3217,0x12);
     write_cmos_sensor(0x0216,0x01);
     write_cmos_sensor(0x0202,0x04);
     write_cmos_sensor(0x0203,0xd0);
     ...
     write_cmos_sensor(0x0101,0x03); // mirror off and flip off
     write_cmos_sensor(0x3904,0x03);
     write_cmos_sensor(0x0343,0x88);
     C2599MIPISENSORDB("[lj c2599]sensor_init end\n");
}
/*Tab A9 code for AX6739A-1971 by pengxutao at 20230714 end*/
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            printk("[lj c2599]c2599 get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 streaming_control(kal_bool enable)
{
	if (enable) {
        ...
        write_cmos_sensor(0xe01d, g_tmp1);
        write_cmos_sensor(0xe020, g_tmp2);
        write_cmos_sensor(0x340f, 0x13);
        //printk("@CSTA_002:g_tmp0=0x%x,g_tmp1=0x%x,g_tmp2=0x%x\n",g_tmp0,g_tmp1,g_tmp2);
	} else {
...
/******************** PDAF END ********************/
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		default:
			...
			/*Tab A9 code for AX6739A-1181 by lisizhou at 20230619 start */
			*feature_return_para_32 = 1; /*BINNING_AVERAGED*/
			/*Tab A9 code for AX6739A-1181 by lisizhou at 20230619 end */
			//-bug 558061, shaozhuchao.wt, modify, 2020/07/23, modify codes for main camera hw remosaic
			break;
		}
...
UINT32 A0902FRONTC2599CXT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
...
//UINT32 A0902FRONTC2599CXT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
//{
   // C2599MIPISENSORDB("[lj c2599]func= %s \n",__func__);
